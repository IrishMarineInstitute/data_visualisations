...
heatmap_data = 
  ...
  select(MeasureCode, GEScomponent, CoordinationLevel, PoliciesConventions, MeasurePurpose, ResponsibleCompetentAuthority,
         DAPSIR, ImplementationStatus, UpdateType, RelevantKTMs, Feature, MeasureType, PolicyNational)
...
heatmap_data %>%
  ...
  guides(fill="none")
ggsave('POM_charts/1. Policy Bubble Chart_v3.jpg')

#### TREEMAP - DAPSIR and Descriptor ####
heatmap_data %>%
  group_by(DAPSIR, GEScomponent) %>%
  summarise(n = as.double(n_distinct(MeasureCode))) %>%
  treemap(
    index = c("GEScomponent", "DAPSIR"),
    vSize = "n",
    vColor = "GEScomponent",
    type='categorical',
    ...
    title='Number of measures for each Descriptor and DAPSIR',
    position.legend='bottom',
    reverse.legend=TRUE,
    title.legend='Descriptor'
  )
ggsave('POM_charts/pom_DAPSIR_descriptor_treemap.jpg')  
...
heatmap_data %>%  
  ...
  coord_polar()
ggsave('POM_charts/6. Circle diagram_v3.jpg')
...
#### STACKED - Implementation Status and Update Type ####
pom_report_net_data %>%
  filter(ImplementationStatus != 'Measure withdrawn') %>%
  ggplot() +
  geom_bar(aes(x=UpdateType, fill=ImplementationStatus)) +
  scale_x_discrete(expand=c(0,0)) +
  scale_y_continuous(expand=c(0,1), limits=c(0,120)) +
  scale_fill_brewer(palette='RdBu') +
  labs(y='Number of Measures',
       x='Update Type',
       fill='') +
  coord_flip() +
  theme_minimal() +
  theme(legend.position = 'top',
        ...
        legend.text.align = 0)
ggsave('POM_charts/13. Measure Status_v3.jpg')
...
#   theme(legend.position="top") +
#   coord_equal()
# ggsave('POM_charts/12. Measure Update Type_v3.jpg')

#### WAFFLE CHART ####
waffle_data = 
  pom_report_net_data %>%
  mutate(DAPSIR = ifelse(is.na(DAPSIR), 'None', DAPSIR)) %>%
  group_by(DAPSIR) %>%
  summarise(n = n_distinct(MeasureCode)) %>%
  mutate(prop = (n/sum(n) * 100)) %>%
  mutate(pct_text = paste(DAPSIR, ' (', as.character(round(prop, 0)), '%)',
                          sep=''))

waffle_vector = round(round(waffle_data$prop, 0)/2.5)
...
ggsave('POM_charts/pom_DAPSIR_waffle.jpg')

#### TREEMAP - Measure Type ####
heatmap_data %>%
  group_by(MeasureType) %>%
  summarise(n = n_distinct(MeasureCode)) %>%
  mutate(prop = paste(MeasureType, '\n',
...
#   theme(legend.position='top',
#         plot.title=element_text(hjust=0.5))
# ggsave('POM_charts/KTMGraph_v3.jpg')
...
heatmap_data %>%
  ...
  theme_minimal() +
  guides(fill="none")
ggsave('POM_charts/9. Features Bubble Chart_v3.jpg')
